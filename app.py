...
# Functional Notes in Sidebar
with st.sidebar.expander("📖 UI Guide & Controls"):
    st.write("""
    **VTG Threshold:** Sets the minimum signal strength required for a target to be displayed.
    **TMI Limit:** Maximum toxicity risk allowed in the results.
    **Neighbors (K):** Adjusts the depth of the interaction network around the Hub.
    """)

min_sig = st.sidebar.slider("Minimum VTG Threshold", 0.0, 3.0, 0.8, help="Filter out targets with low signal potency.")
max_t = st.sidebar.slider("TMI Toxicity Limit", 0.0, 1.0, 0.8, help="Exclude targets exceeding this safety risk level.")

st.sidebar.divider()
search_query = st.sidebar.text_input("🔍 Search Hub Target", help="Type a Gene/Target name (e.g., METTL3) to center the analysis.").strip().upper()
top_k = st.sidebar.slider("Number of Neighbors (K)", 3, 30, 10, help="Number of related targets to display in the network.")
...
if df.empty or ("error" in df.columns):
    st.error("Database Connection Error. Please verify Supabase credentials.")
    st.stop()
...
if not filtered_df.empty:
    ...
    with col_net:
        ...
        # Visual Notes
        st.caption("🟢 Green: Safe | 🟡 Yellow: Caution | 🔴 Red: Toxic | ⚪ Hub: White Border")
    ...
    with col_info:
        ...
        fig_bar = px.bar(
            ...
            template="plotly_dark", labels={"initial_score": "VTG Intensity", "target_id": "Target"}
        )
...
if search_query and not filtered_df.empty:
    ...
    if not hub_row.empty:
        ...
        <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; margin-bottom: 25px;">
            <div title="OMI: Overall Molecular Intelligence" style="background: #111; padding: 15px; border-radius: 8px; border-top: 4px solid #007bff; text-align: center;">
            ...
            <div title="SMI: Synergistic Mapping Interactions" style="background: #111; padding: 15px; border-radius: 8px; border-top: 4px solid #6f42c1; text-align: center;">
            ...
            <div title="VTG: Vital Target Gradient" style="background: #111; padding: 15px; border-radius: 8px; border-top: 4px solid #fd7e14; text-align: center;">
            ...
            <div title="TMI: Toxicity Management Index" style="background: #111; padding: 15px; border-radius: 8px; border-top: 4px solid {tmi_color}; text-align: center;">
            ...
            <div title="CES: Combined Efficacy Score" style="background: #111; padding: 15px; border-radius: 8px; border-top: 4px solid {ces_color}; text-align: center;">
        ...
        st.info(f"**🧬 Biological Context:** {row.get('description_l0', 'Clinical data pending.')}")
        ...
        with c1:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"MAESTRO_Analysis_{search_query}.csv",
                mime="text/csv",
                help="Export all current network data and scores to a CSV file."
            )
        with c2:
            st.caption("Click the button to export the full dataset including VTG, TMI, and CES scores for the Hub and its neighbors.")
...
with exp1:
    with st.expander("🛡️ AXON Intelligence (OMI/BCI)"):
        st.write("**OMI:** Overall Molecular Intelligence. Evaluates the strategic relevance of a target.")
        st.write("**BCI:** Biological Cost Index. Calculates the energy/biological expenditure to inhibit a target.")
with exp2:
    with st.expander("💊 ODI & PMI Systems"):
        st.write("**ODI:** Omikron Drug Inventory. Links targets to known pharmaceutical compounds.")
        st.write("**PMI:** Pathway Management Interface. Maps biological cascades associated with the Hub.")
with exp3:
    with st.expander("🧪 GCI & TMI (Clinical/Safety)"):
        st.write("**GCI:** Global Clinical Inventory. Real-time monitoring of active clinical trials.")
        st.write("**TMI:** Toxicity Management Index. Predicts adverse effects on non-cancerous tissues.")
...
<div style="background-color: #0e1117; padding: 25px; border-radius: 12px; border: 1px solid #333; text-align: center; max-width: 900px; margin: 0 auto; margin-top: 20px;">
    ...
    <p style="font-size: 0.8rem; color: #888; text-align: justify; line-height: 1.5;">
        <b>MAESTRO Omikron Suite</b> is for Research Use Only (RUO). Generated analyses do not replace medical advice. 
        TMI and CES scores are probabilistic models based on AXON datasets.
    </p>
