...
min_sig = st.sidebar.slider(
    ...
    help="VTG Score: Indica la forza del segnale biologico. Più alto è il valore, più il target è espresso/attivo."
)
max_t = st.sidebar.slider(
    "Limite Tossicità TMI", 0.0, 1.0, 0.8,
    help="TMI Index: Valuta il rischio di effetti off-target. Valori bassi indicano maggiore sicurezza."
)
...
<div style="background-color: #1a1a1a; padding: 12px; border-radius: 8px; border-left: 4px solid #ff4b4b; margin-top: 15px;">
    ...
    <p style="font-size: 0.7rem; color: #aaa; text-align: justify; line-height: 1.2;">
        Uso esclusivo di ricerca (Research Use Only). I dati algoritmici non costituiscono diagnosi o parere clinico.
    </p>
</div>
        ...
        if target_data.empty:
            ...
            <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; margin-bottom: 15px;">
                <div title="OMI: Rilevamento molecolare nel database AXON" style="background: #111; padding: 12px; border-radius: 8px; border-top: 4px solid #007bff; text-align: center;">
                    <span style="font-size: 0.7rem; color: #aaa;">OMI</span><br><span style="font-size: 1.1rem; font-weight: bold;">DETECTED</span>
                </div>
                <div title="SMI: Pathway biologici interconnessi rilevati nel PMI" style="background: #111; padding: 12px; border-radius: 8px; border-top: 4px solid #6f42c1; text-align: center;">
                    <span style="font-size: 0.7rem; color: #aaa;">SMI</span><br><span style="font-size: 1.1rem; font-weight: bold;">{len(pmi_df)} Linked</span>
                </div>
                <div title="ODI: Farmaci e molecole attive sul target" style="background: #111; padding: 12px; border-radius: 8px; border-top: 4px solid #ffc107; text-align: center;">
                    <span style="font-size: 0.7rem; color: #aaa;">ODI</span><br><span style="font-size: 1.1rem; font-weight: bold;">{len(odi_df)} Items</span>
                </div>
                <div title="TMI: Toxicity Management Index (0 safe - 1 risk)" style="background: #111; padding: 12px; border-radius: 8px; border-top: 4px solid #dc3545; text-align: center;">
                ...
                <div title="CES: Combined Efficiency Score (Potenziale terapeutico)" style="background: #111; padding: 12px; border-radius: 8px; border-top: 4px solid #28a745; text-align: center;">
                    <span style="font-size: 0.7rem; color: #aaa;">CES</span><br><span style="font-size: 1.1rem; font-weight: bold;">{row['ces_score']:.2f}</span>
                </div>
            </div>
            """, unsafe_allow_html=True)

            st.warning(f"**🧬 Biological Description L0:** {row.get('description_l0', 'Nodo hub critico rilevato. Meccanismo di segnalazione ad alta priorità.')}")
...
with exp1:
    with st.expander("🛡️ AXON Intelligence (OMI/BCI)"):
        st.write("OMI: Molecular Identification. BCI: Biological Cost Index.")
with exp2:
