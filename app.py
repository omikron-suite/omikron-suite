...
# --- 1. CONFIGURAZIONE DASHBOARD (Deve essere la PRIMA istruzione) ---
...
st.sidebar.divider()
search_query = st.sidebar.text_input("🔍 Cerca Target Specifico (es. HER2, PD-1)", "").strip()

# Logica di filtro AXON
if search_query:
...
# --- 6. LAYOUT PRINCIPALE (Grafici AXON) ---
...
# --- 8. INTEGRAZIONE CLINICAL TRIALS (GCI Database) ---
st.markdown("---")
st.subheader("🧪 Clinical Evidence Portal (GCI Database)")

if search_query:
    st.info(f"Ricerca evidenze cliniche per: **{search_query}**")
    try:
        # Query alla tabella clinical_trials su Supabase
        ...
        if not trials.empty:
            ...
            available_cols = [c for c in cols_view if c in trials.columns]
            st.dataframe(trials[available_cols], use_container_width=True)
        else:
            st.warning(f"Nessun trial trovato nel database GCI per '{search_query}'.")
            
    except Exception as e:
        st.error(f"Errore di connessione a Supabase (Tabella clinical_trials): {e}")
else:
    st.write("💡 Digita un target nella barra laterale per visualizzare i dati clinici GCI.")
