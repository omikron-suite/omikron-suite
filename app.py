...
if search_query and not df.empty:
    # 1. Troviamo il target principale
    main_target = df[df['target_id'].str.upper() == search_query]
    
    # 2. Definiamo i "vicini" (Logica simulata per MVP: i 3 target precedenti e successivi nel DB)
    # In un DB reale, qui useremmo una tabella di interazioni 'target_a -> target_b'
...
if not filtered_df.empty:
    ...
    for _, row in filtered_df.iterrows():
        # Nodo con stile distintivo se è quello cercato
        ...
        G.add_node(row['target_id'], 
                   ...
                   border=5 if is_focus else 1)
    
    nodes = list(G.nodes())
    # Colleghiamo tutto al focus target se esiste
