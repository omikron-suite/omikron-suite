...
# --- NUOVA SEZIONE: SIDEBAR 3-COLONNE (GCI | PMI | ODI) ---
if search_query:
    st.sidebar.divider()
    st.sidebar.subheader("📂 Hub Intelligence Data")
    
    # Creazione delle 3 colonne nella Sidebar
    sb_c1, sb_c2, sb_c3 = st.sidebar.columns(3)
    
    with sb_c1:
        st.markdown("**ODI** (Drug)")
        if not odi_df.empty:
            for d in odi_df["Generic_Name"].unique()[:5]: # Limitiamo per spazio
                st.caption(f"💊 {d}")
        else: st.caption("Empty")

    with sb_c2:
        st.markdown("**PMI** (Path)")
        if not pmi_df.empty:
            for p in pmi_df["Canonical_Name"].unique()[:5]:
                st.caption(f"🧬 {p}")
        else: st.caption("Empty")

    with sb_c3:
        st.markdown("**GCI** (Trial)")
        if not gci_df.empty:
            for g in gci_df["Phase"].unique()[:5]:
                st.caption(f"🧪 {g}")
        else: st.caption("Empty")
...
if search_query and not df.empty:
    ...
    if not target_data.empty:
        ...
        # ... (Metrics come nel tuo codice originale)
...
if not filtered_df.empty:
    ...
    fig_net.add_trace(go.Scatter(
        ...
        marker=dict(size=[G.nodes[n]["size"] for n in nodes], color=[G.nodes[n]["color"] for n in nodes], colorscale="RdYlGn_r", showscale=True, line=dict(width=2))
    ))
...
st.divider()
# Tabelle in basso
st.caption("MAESTRO Suite | Sidebar v2 | RUO")
