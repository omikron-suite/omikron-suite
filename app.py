...
st.sidebar.caption("💡 *Sotto 0.8 è rumore di fondo.*")

max_t = st.sidebar.slider("Limite Tossicità (TMI)", 0.0, 1.0, 0.8)
st.sidebar.caption("⚠️ *Sopra 0.7 il rischio è elevato.*")

st.sidebar.divider()
st.sidebar.markdown("### 🔍 Target Intelligence")
search_query = st.sidebar.text_input("Cerca Target Specifico", placeholder="es. KRAS, HER2").strip()
...
# --- 5. RAGNATELA RIPRISTINATA (LOOK ORIGINALE) ---
st.divider()
...
if not filtered_df.empty:
    ...
    for _, row in filtered_df.iterrows():
        G.add_node(row['target_id'], size=float(row['initial_score'])*25, color=float(row['toxicity_index']))
    
    nodes = list(G.nodes())
    # Ripristino densità: ogni nodo prova a connettersi a più vicini (fino a 5)
    for i in range(len(nodes)):
        for j in range(i + 1, min(i + 6, len(nodes))):
            G.add_edge(nodes[i], nodes[j])

    # Layout più compatto (k=0.5 invece di 1.0)
    ...
    edge_trace = go.Scatter(x=edge_x, y=edge_y, line=dict(width=0.8, color='#666'), mode='lines', hoverinfo='none')
...
if search_query:
    ...
    st.info("💡 Digita un biomarker per il cross-referencing clinico.")
